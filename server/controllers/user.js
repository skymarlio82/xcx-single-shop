...
let register = async (ctx, next) => {
  ...
  // 查重
  let isExist = await (
    sequelize.query("select `openId` from user where openId='" + crb.openId + "'", {
      type: sequelize.QueryTypes.SELECT
    })
  );
  console.log("openId isExist: ", isExist);
  // 用户初次登录注册信息
  if (isExist.length === 0) {
    if (!crb.avatarUrl) {
    ...
    return ctx.response.body = { code: 0, msg: "注册成功" };
  } else {
    return ctx.response.body = { code: -2, msg: "该用户已注册" };
  }
};
...
let login = async (ctx, next) => {
  ...
  return ctx.response.body = { code: 0, msg: "登录成功" };
};
